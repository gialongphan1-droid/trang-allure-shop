...
const AdminLayout = () => {
  ...
  return (
    <div className="flex min-h-screen transition-colors bg-background text-foreground">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed top-0 left-0 h-full bg-brand-text dark:bg-gray-900 text-white transition-all duration-300 z-50
        ${isSidebarOpen ? 'w-64 translate-x-0' : 'w-64 -translate-x-full'}
        md:translate-x-0 md:w-64
      `}>
        <div className="flex items-center justify-between p-4 border-b border-white/20 dark:border-gray-700">
        ...
        <div className="px-4 py-2 border-t border-white/20 dark:border-gray-700">
          <Button
            variant="ghost"
      ...
      <main className="flex-1 md:ml-64">
        <header className="sticky top-0 z-30 p-4 transition-colors bg-white shadow-sm dark:bg-gray-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-semibold text-brand-text dark:text-white">
                {menuItems.find(item => window.location.pathname === item.path)?.label || 'Dashboard'}
              </h2>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                ...
                size="sm"
                onClick={viewAsCustomer}
                className="items-center hidden gap-2 md:flex border-brand-primary text-brand-primary hover:bg-brand-primary hover:text-white dark:border-brand-primary dark:text-brand-primary dark:hover:bg-brand-primary dark:hover:text-white"
              >
