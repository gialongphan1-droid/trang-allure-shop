...
const AdminDashboard = () => {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				const [statsResponse, productsResult, categoriesResult, bannersResult] =
					await Promise.all([
	...
	const statsData = [
		...
		{
			title: "Tổng banner",
			value: stats?.totalBanners ?? banners?.length ?? 0,
			icon: Image,
		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
				{statsData.map((stat, index) => (
					<Card
						key={index}
						className="overflow-hidden transition hover:shadow-md dark:bg-gray-800"
					>
			...
			<Card className="dark:bg-gray-800">
				<CardHeader>
					<div className="flex items-center justify-between">
						<CardTitle className="text-brand-text dark:text-white">
							...
							<table className="w-full text-sm">
								<thead>
									<tr className="border-b border-gray-200 dark:border-gray-700">
										<th className="px-4 py-2 text-left text-gray-500 dark:text-gray-400">
											Tên sản phẩm
										</th>
										<th className="px-4 py-2 text-left text-gray-500 dark:text-gray-400">
											Giá
